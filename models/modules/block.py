...
def pad(pad_type, padding, dim=3):
...
def conv_block(in_nc, out_nc, kernel_size, stride=1, dilation=1, groups=1, bias=True, \
               pad_type='zero', norm_type=None, act_type='relu', mode='CNA', dim=3):
    '''
...
class LapSRBlock(nn.Module):
    def __init__(self, dim=3):
        super(LapSRBlock, self).__init__()
        conv_func = nn.Conv2d if not dim == 3 else nn.Conv3d
        convtrans_func = nn.ConvTranspose2d if not dim == 3 else nn.ConvTranspose3d

        self.cov_block = nn.Sequential(
            conv_func(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False),
            ...
            conv_func(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            convtrans_func(in_channels=64, out_channels=64, kernel_size=4, stride=2, padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
        )
...
def pixelshuffle_block(in_nc, out_nc, upscale_factor=2, kernel_size=3, stride=1, bias=True, \
                        pad_type='zero', norm_type=None, act_type='relu', dim=3):
    '''
    Pixel shuffle layer
